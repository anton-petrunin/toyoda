...
If not, then there is a $W$-preserving projection of the whole picture to a  nondegenerate subspace; apply this projection and note that this subspace is isometric a subspace of $\RR^{3,1}$.
...
If a strictly spacelike hyperplane $\Pi$ cuts from $S$ a compact set $Q$ that lies under $\Pi$, then $Q$ is convex in $S$;
...
Each light ray has vanishing length; and it can be identified with its starting point in $\Sigma$. This map $\Lambda^-\to \Sigma^-$ is idenitcal on $\Sigma^-$ and it does not increase lengths of curves, therefore it describes the needed isometry.
...
Further note that $\partial K^-$ can be approximated by smooth convex strictly spacelike hypersurfaces. 
Therefore $\Lambda^-$ admits an approximation by smooth discs $Q_n$ of the type described above.
...
By the lemma every shortest path in $\Sigma^\pm$ between $p$ and $q$ has to lie on $\Pi$ which implies the second statement.
\qeds

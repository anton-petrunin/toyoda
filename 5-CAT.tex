...
Assume $D$ is a smooth spacelike disc in $\RR^{3,1}$
that lies under $\Pi$, has its boundary on $\Pi$.
Suppose that $D$ is convex; that is, $\Pi$ and $D$ cut a convex subset in $\RR^{3,1}$.
By Gauss formula, $D$ has nonpositive sectional curvature.

Recall that Liberman's lemma \cite[Theorem~3]{milka} implies that time coordinate is convex on any geodesic in $D$.
We may assume that time is vanishing on $\Pi$;
therefore, by the lemma, $D$ has a convex boundary.
...
Note that the metric on $D_n$ converges to a pseudometric on $D$ that is induced by $W$.
...
\parit{Proof.}
Since $u$ and $v$ are timelike, $\langle u,u\rangle=\langle v,v\rangle=0$.
